...
'''Transform the word frequency dic to a two-dimension matrix'''
def dic2matrixNorm(dicW):
    # dicW = {ts:[index:wordfreq]}
    X = []
    keyset = dicW.keys() # time interval
    timestamps = sorted(keyset)

    # traverse each time interval
    for ts in timestamps:
        x = [0 for i in range(vocabulary_size)] # create a vector with 5000 zeros
        Sum, Min, Max = 0, sys.maxsize, 0.0
        # find the max and min word frequency in this time interval
        for pair in dicW[ts]:
            wordfreq = int(pair.split(':')[1])
            Sum += wordfreq
            if wordfreq > Max:
               Max = wordfreq
            if wordfreq < Min:
...
def loadData():
    '''
    :return x_word_train: word frequency matrix of training set
    :return y_train: one-hot label of training set
    :return y_gen_train: generator label(contray to y_train)
    :return Len_train: number of tweets in each event
    :return x_word_test: word frequency matrix of test set
    :return y_test: one-hot label of test set
    :return index_true: index of true label event
    :return index_false: index of false label event
    '''
    # load every event label
    ...
    for line in open(textPath):
        ...
        # ts: time interval, Vec: word:freq
    ...
    for eid in open(trainPath):
        ...
        Len_train.append(len(textDic[eid])) # list of training set length
        #wordFreq = dic2matrix( textDic[eid] )
        wordFreq = dic2matrixNorm(textDic[eid]) # word frequency matrix
...
x_word_train = pad_sequence([torch.tensor(seq).float() for seq in x_word_train], batch_first=True).to(device)
y_train = torch.tensor(y_train).to(device)
yg_train = torch.tensor(yg_train).to(device)
# Len_train = torch.tensor(Len_train).to(device)
x_word_test = pad_sequence([torch.tensor(seq).float() for seq in x_word_test], batch_first=True).to(device)
y_test = torch.tensor(y_test).to(device)
# index_true = torch.tensor(index_true).to(device)
# index_false = torch.tensor(index_false).to(device)
...
GANmodel = GANmodel.to(device)
t1 = time.time()
# print(f"GAN model established using {(t1-t0)/60} minutes")
print("GAN model established using {} minutes".format((t1-t0)/60))

## 3. pre-train or load model
if os.path.isfile(modelPath):
   GANmodel = load_model(modelPath, GANmodel).to(device)
   lr_d, lr_g = 0.0001, 0.0001
else:
   # pre train classifier
   if os.path.isfile(modelPath_dis):
      GANmodel = load_model_dis(modelPath_dis, GANmodel).to(device)
      #lr_d = 0.005
   else:
   ...
   if os.path.isfile(modelPath_pre):
      GANmodel = load_model(modelPath_pre, GANmodel).to(device)
