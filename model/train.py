...
def trainRNN_step(flag, model, x_word, Len, y, yg, learning_rate):
    ...
    if flag == "gen_nr":
        # print("training Generator from original non-rumor sample...")
        model.train_gnr(x_word, Len, y, learning_rate)
        #model.train_gnr_pre(x_word, x_index, Len, y, learning_rate)
    if flag == "gen_rn":
        # print("training Generator from original rumor sample...")
        model.train_grn(x_word, Len, y, learning_rate)
        #model.train_grn_pre(x_word, x_index, Len, y, learning_rate)
    if flag == "dis_nr":
        # print("training Discriminator from original non-rumor sample...")
        model.train_d(x_word, y, learning_rate)
        # print("training Discriminator from generated rumor sample Xnr(n -> r)...")
        model.train_dnr(x_word, Len, yg, learning_rate)
        # print("training Discriminator from generated non-rumor sample Xnrr(n -> r -> n)...")
        model.train_dnrn(x_word, Len, y, learning_rate)
    if flag == "dis_rn":
        # print("training Discriminator from original rumor sample...")
        model.train_d(x_word, y, learning_rate)
        # print("training Discriminator from generated non-rumor sample Xrn(r -> n)...")
        model.train_drn(x_word, Len, yg, learning_rate)
        # print("training Discriminator from generated rumor sample Xrnr(r -> n -> r)...")
        model.train_drnr(x_word, Len, y, learning_rate)
...
def train_Gen_Dis(model, x_word, Len, y, yg, index_t, index_f, x_word_test, y_test, lr_g, lr_d, Nepoch, modelPath):
    ...
    ## x_word, x_index: matrix
    ## y: (ground truth) ivector [0, 1] for train generator
    Len = [len(x_word[0]) for _ in Len] # set Len to equal value(pytorch tensor)
    ...
    x_word_t = torch.stack([x for x in x_word_t])
    y_t = torch.stack([x for x in y_t])
    yg_t = torch.stack([x for x in yg_t])
    x_word_f = torch.stack([x for x in x_word_f])
    y_f = torch.stack([x for x in y_f])
    yg_f = torch.stack([x for x in yg_f])
    ...
    for epoch in range(Nepoch):
        ...
        if (epoch+1) % 40 == 0:
            with torch.no_grad():
                ...
                print("{}: train num={} epoch={}: lossg={} lossd={}".format(Time, num_examples_seen, epoch + 1, loss_gen, loss_dis))
        ...
        for k in range(1):
            ...
            random.shuffle(indexs)
            n1, n2 = 0,0
            for j in tqdm(indexs[f*batchsize:(f+1)*batchsize]):
                if j in index_t:
            ...
            #print 'G:',n1, n2
        with torch.no_grad():
            print("Calculating gen(n->r) loss...")
            l_gnr, l_cnr, l_nr = model.calculate_total_loss_gen_cnr(x_word_t, Len_t, y_t)
            print("Calculating gen(r->n) loss...")
            l_grn, l_crn, l_rn = model.calculate_total_loss_gen_crn(x_word_f, Len_f, y_f)
            loss_g, loss_c, loss_gen = (l_gnr + l_grn)/2, (l_cnr + l_crn)/2, (l_nr + l_rn)/2
        ...
        with torch.no_grad():
            ...
            if res[1] > acc_o:
                ...
                acc_o = res[1]
                print("new RES:", res)
            print(f"epoch={epoch + 1}: acc={acc} lg={loss_g} lc={loss_c} ld={loss_d}")
